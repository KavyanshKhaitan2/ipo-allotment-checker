...
def get_options(driver:uc.Chrome):
	options = []
	driver.get("https://www.skylinerta.com/ipo.php")
	companyDropDown = Select(driver.find_element("xpath", "//select[@id='company']"))
...
def get_data(driver:uc.Chrome, options, pans):
	...
	for pan, name in pans:
		...
		for i in options:
			...
			panInput = driver.find_element("xpath", "//input[@id='pan']")
			panInput.send_keys(pan)

			submitBtn = driver.find_element("xpath", "//input[@class='iposearch']")
			submitBtn.click()

			try:
				if driver.find_element("xpath", "//div[@class='fullwidth resultsec']/p[1]/strong").is_displayed():
					print("Found results")
					applied = driver.find_element("xpath", "//tbody/tr[2]/td[1]").text
					alloted = driver.find_element("xpath", "//tbody/tr[2]/td[3]").text
...
if __name__ == "__main__":
	...
	util.convert_to_csv(table, 'skyline.csv')
